...
def _schedule_supplier_repairs(
    ...
    available_slots = _remaining_repair_slots(state=state, policy=policy)
...
def _remaining_repair_slots(state: SimState, policy: PolicySpec) -> int:
    max_parallel = int(policy.params.get("max_parallel_repairs", state.params.priority_repair_max_parallel))
    pending_total = len(state.repair_pending_suppliers) + len(state.repair_pending_items)
    return max(0, max_parallel - pending_total)
