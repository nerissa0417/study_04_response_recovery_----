...
def standardize(raw_bundle: RawBundle, rules: dict | None = None) -> StandardBundle:
    ...
    supplier_input_materials = _standardize_supplier_input_materials(tables)
    supplier_purchased_materials = _standardize_supplier_purchased_materials(tables)
    ...
    if critical_nodes is not None and not critical_nodes.empty:
        items, suppliers = _apply_critical_node_labels(items=items, suppliers=suppliers, critical_nodes=critical_nodes)
        critical_node_selection_mode = "manual_list"
    else:
        items, suppliers = _apply_random_critical_node_labels(items=items, suppliers=suppliers, config=config)
        critical_node_selection_mode = "random_if_no_manual"
    ...
    metadata = {
        ...
        "critical_node_selection_mode": critical_node_selection_mode,
        ...
        "standardized_optional_tables": {
            ...
            "supplier_input_materials": len(supplier_input_materials),
            "supplier_purchased_materials": len(supplier_purchased_materials),
    ...
    return StandardBundle(
        ...
        supplier_input_materials=supplier_input_materials,
        supplier_purchased_materials=supplier_purchased_materials,
...
def _standardize_supplier_input_materials(tables: dict[str, pd.DataFrame]) -> pd.DataFrame:
    source = tables.get("supplier_input_material")
    columns = ["relation_id", "supplier_id", "item_id", "supply_mode", "relation_type"]
    if source is None or source.empty:
        return _empty_frame(columns)
    frame = source.rename(
        columns={
            "Supplier_Input_Material_ID": "relation_id",
            "Supplier_ID": "supplier_id",
            "Input_Material_ID": "item_id",
            "Supply_Mode": "supply_mode",
        }
    )[["relation_id", "supplier_id", "item_id", "supply_mode"]].copy()
    frame["relation_type"] = "supplier_input_material"
    return frame[columns]


def _standardize_supplier_purchased_materials(tables: dict[str, pd.DataFrame]) -> pd.DataFrame:
    source = tables.get("supplier_purchased_material")
    columns = ["relation_id", "supplier_id", "item_id", "relation_type"]
    if source is None or source.empty:
        return _empty_frame(columns)
    frame = source.rename(
        columns={
            "Supplier_Purchased_Material_ID": "relation_id",
            "Supplier_ID": "supplier_id",
            "Purchased_Material_ID": "item_id",
        }
    )[["relation_id", "supplier_id", "item_id"]].copy()
    frame["relation_type"] = "supplier_purchased_material"
    return frame[columns]
...
def _standardize_supplier_edges(tables: dict[str, pd.DataFrame]) -> pd.DataFrame:
    edge_source = tables.get("supplier_activated_network", tables["supplier_network"]).copy()
    edge_id_column = "Supplier_Activated_Network_ID" if "Supplier_Activated_Network_ID" in edge_source.columns else "Supplier_Network_ID"
    edges = edge_source.rename(
        columns={
...
def _apply_random_critical_node_labels(
    *,
    items: pd.DataFrame,
    suppliers: pd.DataFrame,
    config: dict,
) -> tuple[pd.DataFrame, pd.DataFrame]:
    node_cfg = config.get("critical_nodes", {})
    mode = str(node_cfg.get("mode", "random_if_no_manual")).lower()
    if mode in {"none", "manual_only"}:
        return items, suppliers

    items = items.copy()
    suppliers = suppliers.copy()
    count = int(node_cfg.get("random_item_count", 0))
    if count <= 0:
        return items, suppliers

    include_final_product = bool(node_cfg.get("include_final_product", False))
    candidates = items.copy()
    if not include_final_product and "is_final_product" in candidates.columns:
        non_final = candidates.loc[~candidates["is_final_product"].astype(bool)]
        if not non_final.empty:
            candidates = non_final

    if candidates.empty:
        return items, suppliers

    seed = int(node_cfg.get("random_seed", 42))
    chosen_ids = (
        candidates["item_id"]
        .astype(str)
        .sample(n=min(count, len(candidates)), random_state=seed)
        .tolist()
    )
    mask = items["item_id"].astype(str).isin(chosen_ids)
    items.loc[mask, "is_key_node"] = True
    items.loc[mask, "critical_tier"] = "key"
    items.loc[mask, "critical_score"] = 1.0
    items.loc[mask, "critical_reason"] = "random key node selected before manual list is provided"
    items.loc[mask, "critical_source"] = "random_default"
    return items, suppliers
