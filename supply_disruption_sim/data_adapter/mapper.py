...
def list_required_roles(alias_config: dict[str, Any]) -> list[str]:
    return [
        role
        for role, role_config in alias_config["roles"].items()
        if role_config.get("required", True)
    ]


def list_known_roles(alias_config: dict[str, Any]) -> list[str]:
    return list(alias_config["roles"].keys())
