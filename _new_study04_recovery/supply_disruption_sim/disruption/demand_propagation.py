...
def propagate_demand(current_date, state: SimState, model: ModelBundle) -> None:
    items = model.standard_bundle.items.set_index("item_id")
    _reset_item_demand_state(state=state, items=items.index.tolist())
    _propagate_product_demand(current_date=current_date, state=state, model=model, items=items)
...
def _propagate_product_demand(current_date, state: SimState, model: ModelBundle, items) -> None:
