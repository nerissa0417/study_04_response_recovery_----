...
def build_daily_record(current_date: pd.Timestamp, state: SimState, model: ModelBundle) -> dict:
    ...
    record = {
        ...
        "affected_non_key_items": sum(
            item_id not in key_item_ids and status in {"degraded", "unavailable"}
            for item_id, status in state.item_effective_status.items()
        ),
        "failed_non_key_items": sum(
            item_id not in key_item_ids and status == "unavailable"
            for item_id, status in state.item_effective_status.items()
        ),
        "disrupted_non_key_suppliers": sum(
            supplier_id not in key_supplier_ids and status == "disrupted"
            for supplier_id, status in state.supplier_status.items()
        ),
        "degraded_non_key_suppliers": sum(
            supplier_id not in key_supplier_ids and status == "degraded"
            for supplier_id, status in state.supplier_status.items()
        ),
    }
...
def summarize_result(
    ...
    summary = {
        ...
        "max_degraded_key_suppliers": int(history["degraded_key_suppliers"].max()) if "degraded_key_suppliers" in history else 0,
        "max_affected_non_key_items": int(history["affected_non_key_items"].max()) if "affected_non_key_items" in history else 0,
        "max_failed_non_key_items": int(history["failed_non_key_items"].max()) if "failed_non_key_items" in history else 0,
        "max_disrupted_non_key_suppliers": int(history["disrupted_non_key_suppliers"].max()) if "disrupted_non_key_suppliers" in history else 0,
        "max_degraded_non_key_suppliers": int(history["degraded_non_key_suppliers"].max()) if "degraded_non_key_suppliers" in history else 0,
