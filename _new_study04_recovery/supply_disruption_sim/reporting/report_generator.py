...
def generate_report(
    ...
    *,
    ...
    key_node_states_csv = tables_dir / "key_node_states.csv"
    general_node_states_csv = tables_dir / "general_node_states.csv"
    ...
    display_spec_markdown = output_path / "display_spec.md"
    _clear_paths(
        [
            history_csv,
            item_history_csv,
    ...
    rendered_key_node_states_csv, rendered_general_node_states_csv = _export_node_state_views(
        result,
        key_node_states_path=key_node_states_csv,
        general_node_states_path=general_node_states_csv,
    )
    if _includes_full_outputs(profile):
    ...
    artifact_paths = _compact_artifact_paths(
        {
            ...
            "key_node_states_csv": rendered_key_node_states_csv,
            "general_node_states_csv": rendered_general_node_states_csv,
    ...
    dashboard_path = export_dashboard_data(
        ...
        display_spec_path=display_spec_markdown,
    )
    ...
    return ReportArtifacts(
        ...
        display_spec_markdown=display_spec_markdown,
...
def _export_node_state_views(
    result: SimulationResult,
    *,
    key_node_states_path: Path,
    general_node_states_path: Path,
) -> tuple[Path | None, Path | None]:
    snapshot = result.network_snapshots[-1] if result.network_snapshots else None
    node_rows: list[dict[str, object]] = []

    if snapshot is not None:
        snapshot_date = snapshot.get("date")
        for node_key, node in dict(snapshot.get("node_state", {})).items():
            node_rows.append(
                {
                    "date": snapshot_date,
                    "node_key": node_key,
                    ...
                    "is_key_node": bool(node.get("is_key_node", False)),
                    "critical_tier": node.get("critical_tier", "general"),
                    "critical_score": float(node.get("critical_score", 0.0) or 0.0),
                    "critical_reason": node.get("critical_reason", ""),
                    "supply_status": node.get("supply_status"),
                    "demand_status": node.get("demand_status"),
                    "fused_status": node.get("fused_status"),
                    "visual_status": node.get("visual_status"),
                }
            )

    if not node_rows and result.model_bundle is not None:
        suppliers = result.model_bundle.standard_bundle.suppliers.copy()
        items = result.model_bundle.standard_bundle.items.copy()
        for row in suppliers.itertuples(index=False):
            node_rows.append(
                {
                    "date": pd.NaT,
                    "node_key": f"supplier:{row.supplier_id}",
                    ...
                    "supply_status": getattr(row, "supply_state_default", None),
                    "demand_status": getattr(row, "demand_state_default", None),
                    "fused_status": getattr(row, "fused_state_default", None),
                    "visual_status": getattr(row, "visual_status_default", None),
                }
            )
    ...
    node_state_df = pd.DataFrame(node_rows)
    node_state_df = node_state_df.sort_values(
        by=["is_key_node", "node_type", "critical_score", "node_id"],
        ascending=[False, True, False, True],
    ).reset_index(drop=True)

    key_nodes = node_state_df.loc[node_state_df["is_key_node"]].copy()
    general_nodes = node_state_df.loc[~node_state_df["is_key_node"]].copy()

    rendered_key_path = None
    rendered_general_path = None
    if not key_nodes.empty:
        key_nodes.to_csv(key_node_states_path, index=False)
        rendered_key_path = key_node_states_path
    if not general_nodes.empty:
        general_nodes.to_csv(general_node_states_path, index=False)
        rendered_general_path = general_node_states_path
    return rendered_key_path, rendered_general_path
