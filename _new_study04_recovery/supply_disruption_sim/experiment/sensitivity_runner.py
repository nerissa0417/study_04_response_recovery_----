...
def _apply_single_source_ratio(bundle: StandardBundle, target_ratio: float, random_seed: int) -> None:
    ...
    current_multi_items = {
        item_id
        for item_id, count in supply_map.loc[supply_map["is_primary"]].groupby("item_id").size().to_dict().items()
        if int(count) > 1
    }
    if len(current_multi_items) > target_multi_count:
        ...
        for item_id in sorted(current_multi_items - keep_multi_items):
            rows = supply_map.loc[(supply_map["item_id"] == item_id) & (supply_map["is_primary"])].sort_values(
...
def _measure_single_source_ratio(bundle: StandardBundle) -> float:
    ...
    primary_counts = bundle.supplier_item_map.loc[bundle.supplier_item_map["is_primary"]].groupby("item_id").size()
    single_source_count = int((primary_counts <= 1).sum()) + max(0, item_count - int(primary_counts.shape[0]))
