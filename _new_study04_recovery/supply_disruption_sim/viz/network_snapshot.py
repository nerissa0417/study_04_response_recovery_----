...
def select_snapshot_points(result: SimulationResult) -> dict[str, dict]:
    ...
    peak_row = history.sort_values(
        by=["fused_failed_items", "fused_affected_items", "system_service_level"],
        ascending=[False, False, True],
    ).iloc[0]
    t_peak = peak_row["date_str"]
    recovery_candidates = history.loc[
        (history["date"] >= result.scenario.start_date.normalize())
...
def render_network_snapshot(
    *,
    ...
    node_legend_ax = fig.add_axes([0.83, 0.60, 0.14, 0.18])
    status_legend_ax = fig.add_axes([0.83, 0.34, 0.14, 0.24])
